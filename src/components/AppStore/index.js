...
class AppStore extends Component {
  state = {searchInput: '', activeTabId: tabsList[0].tabId, appsLists: appsList}

  getFilteredList = () => {
    const {activeTabId, appsLists} = this.state
    const filteredList = appsLists.filter(each => each.category === activeTabId)
    return filteredList
  }
  ...
  render() {
    ...
    const searchResults = filteredList.filter(each =>
